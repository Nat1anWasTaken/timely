...
export function getExtendedMonthBoundaries(year: number, month: number) {
    const startOfPreviousMonth = new Date(year, month - 1, 1);
    const endOfNextMonth = new Date(year, month + 2, 0);

    return {
        start_timestamp: Math.floor(startOfPreviousMonth.getTime() / 1000).toString(),
        end_timestamp: Math.floor(endOfNextMonth.getTime() / 1000).toString()
    };
}
...
export function createExtendedQueryKey(year: number, month: number) {
    return ['calendar-events-extended', year, month];
}